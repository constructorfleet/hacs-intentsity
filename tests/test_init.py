...
async def test_async_initialize_registers_api(hass, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        intentsity.websocket, "async_register_commands", _register_commands
    )
...
def test_intent_output_targets() -> None:
    ...
    assert (
        intentsity._intent_output_targets({"response": {"data": {"targets": []}}})
        is None
    )
    ...
    payload = intentsity._intent_output_targets(
        {
            "response": {
                "data": {"targets": ["light.kitchen"], "success": [], "failed": []}
            }
        }
    )
    assert payload == {
