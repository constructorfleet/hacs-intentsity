...
- **Domain:** `intentsity`
- **Title:** Intentsity
- **Repository:** constructorfleet/hacs-intentsity

**Key directories:**

- `custom_components/intentsity/` - Main integration code
...
- Integration logs: `custom_components.intentsity: debug` in `config/configuration.yaml`
...
- Use the domain `intentsity` for all DOMAIN references
- Prefix all integration-specific classes with `Intentsity`
- Use "Intentsity" as the display title
...
- Inherit from platform base + `IntentsityEntity`
...
- `tests/` mirrors `custom_components/intentsity/` structure
