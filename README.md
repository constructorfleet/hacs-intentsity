...
## Usage
- The sidebar panel shows 100 conversations per page by default (adjustable up to 500), with next/previous pagination.
- Conversations are grouped by ID, showing both user and assistant messages with timestamps.
- Filter conversations by corrected status, the last N days, or a date/time range to focus reviews.
...
- Developers can access the logs via WebSocket:
	- `intentsity/chats/list` with `limit`, optional `offset`, and optional `corrected`, `start`, `end` filters to fetch snapshots. Responses include `total` for pagination.
