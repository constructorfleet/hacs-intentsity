...
This document describes the technical architecture of the Intentsity custom component for Home Assistant.

## Directory Structure

```text
custom_components/intentsity/
...
│   ├── __init__.py          # Exports IntentsityDataUpdateCoordinator
...
│   ├── __init__.py          # Exports IntentsityEntity
...
- `base.py` - Main coordinator class (`IntentsityDataUpdateCoordinator`)
...
**Key class:** `IntentsityDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `IntentsityConfigFlowHandler` (main flow)
- `IntentsityOptionsFlow` (options)
...
**Key class:** `IntentsityEntity` (in `entity/base.py`)
...
2. `IntentsityEntity` for common functionality
...
1. Create directory: `custom_components/intentsity/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `IntentsityEntity`
