...
This document describes all configuration options and settings available in the Intentsity custom integration.
...
```yaml
service: intentsity.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: intentsity.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/constructorfleet/hacs-intentsity/issues) - Report problems
